...
> **Поправка при реализации.** Код ниже отмерял тридцать секунд от постановки
> задачи до результата — то есть считал ожидание вместе с работой. Две минуты
> речи распознаются на видеокарте дольше тридцати секунд, поэтому в облако
> уходила бы каждая длинная диктовка при полностью свободном и исправном
> компьютере, молча тратя деньги каждый день.
>
> В готовом [server/src/agent/queue.js](../../../server/src/agent/queue.js)
> отмеряется только **ожидание в очереди**: `throughAgent` возвращает
> `{ started, done, abandon }`, а `withSpill` ждёт `started`. Взялся агент за
> дело — ждём столько, сколько нужно. Не взялся — `abandon()`, и задача уходит
> облаку, а разбуженная очередь её уже не выполняет.
