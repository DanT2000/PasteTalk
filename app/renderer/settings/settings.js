...
async function start() {
  ...
  // Настройки расставлены — окно можно показывать. Дальше идут опрос
  // движка, список микрофонов и связь с сервером: на холодном запуске это
  // секунды, и ждать их значит показать окно по таймеру, полупустым.
  api.app.ready?.();
  await refreshHealth();
...
// Окно настроек закрывается по-настоящему, но пока оно открыто — живёт.
// Вернулись к нему или сменился состав мониторов — список перечитывается.
...
start()
  ...
  // Окно ждёт этого сигнала, чтобы не показаться полупустым. Шлём и
  // после сбоя: лучше окно с ошибкой на виду, чем пустой экран.
  .finally(() => api.app.ready?.());
