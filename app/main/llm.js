...
const MODES = {
  clean:
    'Убери слова-паразиты, повторы и оговорки. Сохрани смысл, порядок мыслей и манеру речи. '
    + 'Ничего не добавляй от себя и не переписывай то, что и так сказано ясно.',
  ...
  structure:
    'Перепиши текст так, чтобы его было легко читать. Разреши себе менять формулировки и порядок: '
    + 'собери разрозненные куски одной мысли вместе, даже если в речи они шли вразнобой, '
    + 'выстрой изложение логично — от главного к подробностям, раздели на абзацы, '
    + 'а перечисления оформи списком. Устную интонацию замени на письменную. '
    + 'Ни одного факта не потеряй и ни одного не придумай.',

  both:
    'Сначала убери слова-паразиты, повторы и оговорки. Затем перепиши получившееся так, чтобы читалось легко: '
    + 'переформулируй, где выйдет яснее, собери разбросанные куски одной мысли вместе, '
    + 'выстрой логичный порядок, раздели на абзацы, перечисления оформи списком. '
    + 'Смысл сохрани полностью: ничего не теряй и ничего не добавляй.',
};
...
const LANGUAGE_NAMES = {
  ru: 'русский', en: 'английский', uk: 'украинский', de: 'немецкий', fr: 'французский',
  es: 'испанский', it: 'итальянский', pl: 'польский', tr: 'турецкий', zh: 'китайский',
  ja: 'японский', ko: 'корейский', ar: 'арабский', pt: 'португальский', kk: 'казахский',
};

/**
 * Перевод — та же языковая модель, что чистит речь.
 *
 * Отдельный переводчик ставить незачем: у человека уже подключён либо
 * локальный сервер, либо агент по подписке, и качество там выше, чем у
 * оффлайн-словаря на пару сотен мегабайт. Язык источника не указываем —
 * модель определяет его сама, ей для этого хватает самого текста.
 */
async function translate(text, target = 'ru', overrides = {}) {
  const clean = String(text || '').trim();
  if (!clean) return clean;
  ...
  const instruction =
    `Переведи текст на ${language} язык. Язык оригинала определи сам. `
    + 'Сохрани разбивку на строки, числа, имена и единицы измерения. '
    + 'Если текст уже на нужном языке — верни его без изменений. '
    + `${TAIL}`;

  const result = settings.preset.kind === 'cli'
    ? await runCli({ ...settings, mode: 'custom', prompt: instruction }, clean, timeoutMs)
    : await chat({ ...settings, mode: 'custom', prompt: instruction }, clean, timeoutMs);

  log.info(`${settings.preset.title}: переведено на ${language} за ${Date.now() - started} мс`);
  return result;
}
...
module.exports = { improve, translate, models, check, detect, PROVIDERS, MODES, LANGUAGE_NAMES };
