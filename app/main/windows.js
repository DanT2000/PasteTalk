...
function createSettings() {
  ...
  // Запас на холодный запуск: на первом открытии после загрузки Windows
  // страница успевает расставить настройки за две-три секунды, и полутора
  // секунд не хватало — окно показывалось по таймеру, полупустым.
  win.once('ready-to-show', () => setTimeout(() => reveal('страница молчит — по таймеру'), 4000));
