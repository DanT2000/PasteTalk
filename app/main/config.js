...
const DEFAULTS = {
  ...
  appearance: {
    ...
    // Отпечаток закреплённого экрана: имя, размер, положение. Номера
    // экранов Windows перераздаёт после перезагрузки — по отпечатку
  ...
  ai: {
    ...
    // Адреса тоже по провайдерам: переключился со «Своего» на другой и
    // обратно — адрес на месте, а не стёрт.
    urls: {},
    // Запасной провайдер: основной не ответил — текст идёт ему. У «Своего»
    // здесь свои адрес и ключ, у именитых ключ общий (ai.keys).
    backup: { enabled: false, provider: '', baseUrl: '', key: '', model: '' },
    mode: 'clean',
  ...
  engine: {
    ...
    // Спать целиком: после выгрузки модели процесс движка всё равно держит
    // ~2 ГБ (контекст CUDA, ядра cuBLAS/cuDNN) — их отдаёт только выход
    // из процесса. Сон гасит движок после простоя и будит при следующей
    // диктовке; звук при этом копится с первой секунды.
    deepSleep: true,
    ...
    port: 0,
    // Куда складывать модели. Пусто — рядом с настройками, как всегда.
    // Модели весят гигабайты, а системный диск часто забит — поэтому
    // папку можно вынести куда угодно, хоть на другой диск.
    modelsDir: '',
  },
